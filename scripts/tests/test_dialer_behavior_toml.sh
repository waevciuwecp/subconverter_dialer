...
name = "dialer"
type = "select"
rule = ["[]dialer-select", "[]dialer-lb", "[]DIRECT"]
...
(
  cd "$tmp_dir"
  "$bin_path" -f "$pref_path" -g --artifact dialer_toml_case >/dev/null 2>&1
)
...
echo "PASS: toml dialer/provider behavior is correct"
